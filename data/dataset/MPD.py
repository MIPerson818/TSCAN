...
class MPD_dataset(Data.Dataset):
    """
    Palm dataset for person re-identification with open-set protocol.
    """
    def __init__(self, data_path=None, transform=None, split='train', 
                 train_ratio=0.8, domain='HW', test_with_labels=True):
        if not split in ['train', 'test']:
            raise Exception('Invalid dataset split.')
        if not (domain in ["HW", "Mi"] or domain == ["HW", "Mi"]):
            raise Exception('Invalid data source.')
            
        self.transform = transform
        self.loader = default_loader
        self.split = split
        self.data_path = data_path if data_path is not None else '/home/repository/PalmDatas/MPD'
        self.domain = domain if isinstance(domain, list) else [domain]  # 统一转为列表处理
        self.train_ratio = train_ratio
        self.seed = 42
        self.test_with_labels = test_with_labels

        # 接收四个返回值：图像路径、标签、类别到索引映射、索引到类别映射
    ...
    def get_all_image(self, path, domain, split, train_ratio, seed, test_with_labels):
        ...
        # 1. 收集所有类别（格式：device_folder_person_folder）
        class_list = []
        for device_folder in domain:
            device_path = ospj(path, device_folder)
            if not os.path.isdir(device_path):
                raise FileNotFoundError(f"Device path not found: {device_path}")
            for person_folder in os.listdir(device_path):
                person_path = ospj(device_path, person_folder)
                if os.path.isdir(person_path):
                    class_name = f"{device_folder}_{person_folder}"
                    class_list.append(class_name)  # 类别名称唯一标识（设备+人员）
        ...
        # 4. 为当前split的类别创建索引映射（仅包含used_classes）
        class_to_idx = {}
        idx_to_class = {}
        ...
        for class_name in used_classes:
            # 解析类别名称中的设备和人员文件夹
            device_folder, person_folder = class_name.split('_', 1)  # 拆分device和person
            person_path = ospj(path, device_folder, person_folder)
            # 获取该类别下的所有图像
            image_files = [
...
def main():
    ...
    # 配置
    data_path = '/home/repository/PalmDatas/MPD'
    ...
    dataSource = ["HW", "Mi"]  # 可选择单个设备或多个
    train_ratio = 0.8

    # 创建数据集
    train_dataset = MPD_dataset(
        data_path=data_path,
        transform=transform,
        split='train',
        domain=dataSource,
    ...
    print("\n训练集中每个类别的图像数量：")
    for label_idx, count in sorted(train_class_count.items())[:1]:  # 只打印第一个示例
        class_name = train_dataset.idx_to_class[label_idx]
        print(f"类别索引 {label_idx}: {count} 张图像, 类别名称: {class_name}")

    print("\n测试集中每个类别的图像数量：")
    for label_idx, count in sorted(test_class_count.items())[:1]:  # 只打印第一个示例
