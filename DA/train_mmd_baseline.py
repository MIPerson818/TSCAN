...
def main():
    args = build_args('Train MMD baseline')
    ...
    criterion = MMDLoss(kernel_type=str(getattr(opts, 'mmd_kernel_type', 'linear'))).to(device)
    ...
    for epoch in tqdm(range(int(opts.epochs)), desc='MMD'):
        model.train()
        ...
        mmd_weight = float(getattr(opts, 'lambda_mmd', 0.1)) * min(
            1.0,
            (epoch + 1) / max(1, int(getattr(opts, 'mmd_ramp_epochs', 1))),
        )
        ...
        for _ in range(steps):
            ...
            with torch.amp.autocast(device_type='cuda', enabled=(device.type == 'cuda')):
                ...
                mmd_loss = criterion(src_pooled, tgt_pooled)
                total_loss = float(getattr(opts, 'lambda_s', 1.0)) * source_loss + mmd_weight * mmd_loss
            ...
            epoch_mmd_loss += float(mmd_loss.item())
        ...
        record = {
            ...
            'train': {
                ...
                'mmd_loss': round(avg_mmd_loss, 4),
                'total_loss': round(avg_total_loss, 4),
                'lambda_mmd': round(float(mmd_weight), 4),
            },
